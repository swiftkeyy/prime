...
PRIME_LAB_LOCKED = """🧪 <b>PRIME Lab доступен только с PRIME PASS</b>

Этот раздел усиливает продукт и открывает премиальные сценарии:
• дропы дня
• топ недели
• drop alerts
• брендовые коллекции
• расширенную витрину username

Открой PRIME PASS, чтобы получить доступ."""

CUSTOM_SEARCH_LOCKED = """✨ <b>Подбор по слову доступен только с PRIME PASS</b>

Этот режим даёт более сильный и дорогой сценарий подбора username под имя, бренд и стиль.

Открой PRIME PASS, чтобы пользоваться персональным подбором."""

STYLE_LOCKED = """🎨 <b>Стили и расширенные фильтры доступны только с PRIME PASS</b>

Base-режим оставляет базовый поиск.
PRIME открывает стили, более гибкую подачу и сильнее сегментирует выдачу."""
...
Сейчас нет заранее проверенных вариантов под эту основу.
Бот не будет отдавать непроверенные username, чтобы не показывать мусор.

Попробуй другую основу или повтори позже."""
...
def check_rate_limited(retry_after: int = 0) -> str:
    ...
    return f"""⏳ <b>Telegram ограничил live-проверку</b>

Бот не будет выдавать непроверенные username.
Live-проверка получила лимит Telegram.

Повтори поиск {wait}. Для продакшена лучше держать включённым заранее проверенный пул username, а live-проверку оставить выключенной."""
